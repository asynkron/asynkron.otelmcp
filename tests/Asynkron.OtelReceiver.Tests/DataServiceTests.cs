...
public class DataServiceTests
{
    ...
    public async Task GetTrace_ReturnsSpansAndLogsForTraceId()
    {
        ...
        var traceIdBytes = Enumerable.Range(246, 16).Select(i => (byte)i).ToArray();
        var spanId1Bytes = Enumerable.Range(180, 8).Select(i => (byte)i).ToArray();
        var spanId2Bytes = Enumerable.Range(188, 8).Select(i => (byte)i).ToArray();
        var traceIdHex = Convert.ToHexString(traceIdBytes);
